...
//      BlobValue(TempMenBlobCache.memBlobCache(url.asCanonicalString))
    } else {
      val blob = url.createBlob()
      BlobFactory.memBlobCache.put(url.asCanonicalString, blob)
//      TempMenBlobCache.memBlobCache.put(url.asCanonicalString, blob)
      BlobValue(blob)
    }
  }
...
//    TempMenBlobCache.memBlobCache.contains(url)
  }
}